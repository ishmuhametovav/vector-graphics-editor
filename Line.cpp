...
void line::draw_selection_box(TCanvas* canvas, const coordinate_system* coord_system)
{
	//бшмеярх б наыхи лернд
	canvas->Pen->Width = 1;
...
bool line::is_clicked(const double x, const double y) const
{
...
bool line::is_border_clicked(const double x, const double y) const
{
	double max_x = std::max(x1, x2);
	double max_y = std::max(y1, y2);
